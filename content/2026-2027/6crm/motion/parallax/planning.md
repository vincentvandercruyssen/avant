...
2. **Inspiratie & pinboard:** Doelgericht zoeken naar complementair bronmateriaal en referenties:
   ...
   * Krachtig voorgrondobject of personage
   * Typografische stijl (quotes, lettertypes en hiërarchie)  
   Aanmaken van een digitaal pinboard ter inspiratie en visuele voorbereiding.
3. **Mappenstructuur opzetten:** Professionele projectmap aanmaken in OneDrive onder `VoornaamA_Parallax/` met de vereiste submappen:
   * `01_assets/` (originele bronfoto's, fonts)
   * `02_exports/` (definitieve video-export)
   * `VoornaamA_Parallax-Ontwerp.psd` (gelaagd PSD-bestand)
   * `VoornaamA_Parallax-Animatie.aep` (After Effects-animatieproject)
4. **Photoshop hoofddocument & niet-destructieve beeldbewerking:**
